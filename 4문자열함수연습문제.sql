...
SELECT FIRST_NAME, INSTR(FIRST_NAME,'a') FROM EMPLOYEES; --a가있는위치를반환
--SUBSTR 문자열 자르기
SELECT FIRST_NAME,SUBSTR(FIRST_NAME,3) ,SUBSTR(FIRST_NAME,3,2) FROM EMPLOYEES; --3미만 절삭 3번째 위치에서 2개자름

-- CONCAT
SELECT FIRST_NAME || LAST_NAME, CONCAT(FIRST_NAME,LAST_NAME) FROM EMPLOYEES;

-- LPAD, RPAD - 범위를 지정하고, 특정문자로 채움
SELECT LPAD('ABC',10,'*') FROM DUAL; -- ABC를 10칸 잡고, 나머지 부분은 왼쪽붙터*채움
...
SELECT RPAD(SUBSTR(FIRST_NAME,0,3),LENGTH(FIRST_NAME),'*') AS NAME , LPAD(SALARY,10,'*') AS SALARY FROM EMPLOYEES WHERE LOWER(JOB_ID)=LOWER('IT_PROG');
