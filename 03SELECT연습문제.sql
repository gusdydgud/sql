...
SELECT FIRST_NAME,SALARY,DEPARTMENT_ID FROM EMPLOYEES WHERE DEPARTMENT_ID =60 ORDER BY SALARY DESC;
...
SELECT FIRST_NAME ||' '|| LAST_NAME|| ' 사원의 급여는 ' ||SALARY||'달러입니다' AS 돈 FROM EMPLOYEES WHERE LAST_NAME='King'AND FIRST_NAME='Steven';
--14. 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 출력하세요. (컬럼:job_id)
SELECT FIRST_NAME,JOB_ID FROM EMPLOYEES WHERE JOB_ID LIKE '%MAN%';
--15. 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 직무아이디 서대로 출력하세요.
