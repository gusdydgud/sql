...
CREATE SEQUENCE DEPTS_SEQ
    INCREMENT BY 1 --몇씩 증가
    START WITH 1 --시작점
    MAXVALUE 10 --최대지점
    NOCACHE --캐시에 시퀀스를 두지 않음
    NOCYCLE; --최대값에 도달했을때 재사용X
...
--시퀀스 사용방법 2개
SELECT DEPTS_SEQ.CURRVAL FROM DUAL; --현재 시퀀스 (NEXTVAL가 선행되어야함)
...
--만약 시퀀스를 초기화 해야한다면
--시퀀스의 증가값을 -음수로 만들어서 초기화 인것처럼 쓸 수는 있따
SELECT DEPTS_SEQ.CURRVAL FROM DUAL;

--1. 시퀀스의 증가를 -음수로 바꿈
ALTER SEQUENCE DEPTS_SEQ INCREMENT BY -23;
--2. 현재 시퀀스를 전진
...
--INDEX종류는 고유인덱스, 비고유인덱스가 있다
--UNIQUE한 컬럼에는 UNIQUE인덱스(고유) 인덱스가 쓰입니다
--일반컬럼에는 비고유 인덱스를 지정할 수 있습니다
--INDEX는 조회를 빠르게 하지만, DML구문이 많이 사용되는 컬럼은 오히려 성능저하를 부를지도?
